...
public class Schedule { // Schedule is the timetable set to a User/UserGroup
  // This class is used to check if the user is operating under an allowed time period
  private final LocalDate fromDate; // Starting date of the schedule
  private final LocalDate toDate; // Ending date of the schedule
  private final ArrayList<String> weekdays; // Set of allowed weekdays in the schedule
  ...
  public Schedule(ArrayList<String> dateInterval, ArrayList<String> weekdays, ArrayList<String> timeInterval) {
    // Dates, weekdays, and hours are sent as strings so we parse them into appropiate time formats
    ...
    // We don't need to parse weekdays through any library as they are always strings (kinda)
    this.weekdays = weekdays;
  }

  public boolean isInSchedule(LocalDateTime now){ // This function checks if the hours, dates, and weekdays are in order
    LocalDate nowDate = now.toLocalDate(); //we trim the current LocalDateTime to LocalDate
    LocalTime nowTime = now.toLocalTime(); //we trim the current LocalDateTime to LocalTime
